...
import warnings
import moco.builder
from moco.resnet import Bottleneck, ResNetComFea
...
parser.add_argument('-data', type=str, default='/root/data1/tzs/datasets/Imagenet/',
...
parser.add_argument('--schedule', default=[65, 90, 110], nargs='*', type=int,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = ResNetComFea(block=Bottleneck, layers=[3, 4, 6, 3], args=args)
    ...
    # Data loading code
    aug_resize = 224 if args.pretraining_stage else 256
    ...
    train_dataset = datasets.ImageFolder(
        ...
        transforms.Compose(
            [
                transforms.RandomResizedCrop(aug_resize),
        ...
        datasets.ImageFolder(
            ...
            transforms.Compose(
                [
                    ...
                    transforms.CenterCrop(aug_resize),
                    transforms.ToTensor(),
                    normalize,
                ]
            ),
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, losses_rate, losses_rate_y, losses_rate_z,top1, top5],
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output
        output, fea, omni_fea, IF_y_prob, omni_fea_hat, comp_y_prob, comp_z_prob = model(images,training=False,use_codec=True)
...
def validate(val_loader, model, criterion, args):
    ...
    top5 = AverageMeter("Acc@5", ":6.2f")
    progress = ProgressMeter(
        len(val_loader), [batch_time, losses, losses_rate, losses_rate_y, losses_rate_z, top1, top5], prefix="Test: "
    )
    ...
    with torch.no_grad():
        ...
        print(
            " * {losses_rate.avg:.3f} Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}".format(losses_rate=losses_rate, top1=top1, top5=top5)
        )

    return top1.avg


def save_checkpoint(state, is_best, epoch,filename='checkpoint'):
    filename=os.path.join('cls_test3',filename+'_epoch{}'.format(epoch)+'.pth.tar')
    if not os.path.exists('cls_test3'):
        os.makedirs('cls_test3', exist_ok=True)
