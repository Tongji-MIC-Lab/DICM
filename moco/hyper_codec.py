    ...
    def get_mask(self, b, h, w):
        ...
        if self.training:
            if (self.train_scan_mode == 'random' and hasattr(self, 'sampler')):
                ...
                mask = repeat(mask_random.unsqueeze(0), '() n -> d n', d=n) & torch.tril(
                    torch.ones((n, n))).bool() | torch.eye(n).bool()
...
class TransHyperScale(TransDecoder):
    ...
    def build(self):
        # Head projection and out projection if needed
        self.to_patch_embedding = nn.Linear(self.cin, self.config.dim) if self.cin else nn.Identity()
        self.mlp_head = nn.Sequential(nn.LayerNorm(self.config.dim),
                                      nn.Linear(self.config.dim, self.cout)) if self.cout else nn.Identity()
        ...
        if (self.down):
            ...
            self.scale_blocks = clones(UpPixelShuffle(self.config.dim, self.config.dim, kernel_size=3, scale=2),
                                       self.scale)
